...
**age_groupのスタージェンの公式による最適化:**
- **スタージェンの公式**: k = 1 + 3.322 × log₁₀(n)
- データ数 n = 27,128 の場合、最適ビン数 k ≈ 16
- 従来の恣意的な6グループ分割から、統計的に最適な16グループへ変更
- 年齢範囲 18-95歳を等幅で16分割（約4.81歳ごと）
- メリット:
  - データの分布を考慮した客観的なグルーピング
  - より細かい年齢層の特徴を捉えられる
  - 過度な細分化を避けつつ情報を保持
